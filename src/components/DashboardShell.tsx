...
export default function DashboardShell({
  ...
  return (
    <section className="container-pad grid gap-8 py-12 lg:grid-cols-[250px_1fr]">
      <aside className="lg:sticky lg:top-24 lg:h-fit space-y-4">
        ...
        <nav
          className="flex flex-wrap gap-2 lg:flex-col lg:space-x-0 lg:space-y-1.5"
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-[11px] sm:text-sm font-bold transition-all duration-300 ${
                  isActive
